...
ДАТЫ — не выдуманы. Обе группы статей добавлялись в сайт двумя реальными
коммитами: BLOG_RF (16 статей) — 31.07.2026 «Гео на всю Россию + 16 статей
блога», BLOG_BASE (6 статей) — 20.08.2026 «Переписаны 6 тонких статей блога»
(рерайт коротких заготовок в полноценные материалы). Обе даты сверены
`git log --diff-filter=A` по факту, не проставлены на глаз.
...
# Дата = дата коммита, которым эта статья реально попала на сайт (см. докстринг).
PUB_DATE = {}
for _s, *_ in BLOG_BASE:
    PUB_DATE[_s] = "2026-08-20"
for _s, *_ in BLOG_RF:
    PUB_DATE[_s] = "2026-07-31"
