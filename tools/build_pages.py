...
# ---------------------------------------------------------------- данные
# Цены — реальный прайс-лист Аркона (собственный парк) от 19.03.2026,
# см. arkona-price-list-2026-03-19.md. До 03.09.2026 здесь были угаданные
# ориентировочные цены без источника; правка — по прямому запросу
# пользователя («цена по запросу» у конкурентов раздражает, нужна
# реальная цена сразу, даже высокая — лишь бы не выдуманная»).
# Для каждого тоннажа взят самый дешёвый вариант из прайса (минимальная
# длина стрелы/база, не вездеход, если он дороже) — так же, как раньше
# работало "от". 90/100/150/250 т раньше были «под запрос» — теперь есть
# реальные цифры.
AVTOKRAN = [(25,24000),(32,28000),(40,45400),(50,52200),(60,68200),(70,81600),(90,108800),(100,134000),(150,174200),(250,263600)]

TYPES = [
  dict(slug="avtokrany", name="Автокраны", one="автокрана", price=24000,
       meta="25 – 300 тонн",
       lead="Автокраны грузоподъёмностью от 25 до 300 тонн. Своя техника и проверенная сеть, опытные крановщики в стоимости смены, подача от 1 дня по Москве и области."),
  dict(slug="avtovyshki", name="Автовышки", one="автовышки", price=19200,
       meta="высота подъёма 18 – 60 м",
       lead="Автовышки и телескопические подъёмники с высотой подъёма от 18 до 60 метров. Работа на высоте, монтаж, обслуживание фасадов и сетей. Оператор и топливо включены."),
  dict(slug="manipulyatory", name="Манипуляторы", one="манипулятора", price=20300,
  ...
  dict(slug="ekskavatory", name="Экскаваторы", one="экскаватора", price=19600,
       meta="мини, колёсные, гусеничные, погрузчики",
       lead="Экскаваторы и экскаваторы-погрузчики: мини от 3 т, колёсные и гусеничные до 33 т, с ковшом и гидромолотом. Земляные работы, разработка грунта, снос."),
  dict(slug="samosvaly", name="Самосвалы", one="самосвала", price=20000,
  ...
  dict(slug="traly", name="Тралы и негабарит", one="трала", price=48000,
...
# VYSHKI, EKSK, SAMOSVAL — реальные цены Аркона (arkona-price-list-2026-03-19.md).
# Автовышка 50 м намеренно осталась None: в прайсе есть 45 м (37 600) и сразу
# 60 м (64 200), а 50 м как отдельного класса машины у Аркона нет — ставить сюда
# число значило бы придумать его. Честнее «под запрос», как для редких тоннажей.
VYSHKI = [(18,19200),(21,20000),(28,21400),(32,24000),(40,32960),(45,37600),(50,None)]
# MANIP обновлён 05.09.2026: пользователь подтвердил пересчитать пропорционально
# от единственной цифры Аркона (7т = 25 600 ₽/смена, реальная), раз полного
# прайса по остальным тоннажам манипуляторов нет. Коэффициент = 25600/12500
# (старая угаданная цена 7т) = 2.048, применён ко всей лестнице и округлён до
# сотен. Это ОЦЕНКА, а не подтверждённая Аркона цифра (кроме самого 7т) —
# честно, но раз лестница иначе выглядела бы сломанной (7т дороже 8т), это
# лучше, чем оставить только один класс поднятым, а остальные заниженными.
MANIP  = [(3,20300),(5,22300),(7,25600),(8,30500),(10,36700),(12,None),(15,None)]

# Автокраны по длине стрелы: цена зеркалит AVTOKRAN по тому же классу тоннажа
# (см. комментарий там же) — просто другой вход на ту же цену.
STRELA = [(21,24000,25),(28,28000,32),(30,28000,32),(34,45400,40),(40,52200,50),
          (46,68200,60),(50,81600,70),(56,108800,90),(60,134000,100),(70,174200,150)]

# Экскаваторы: (slug, название, ковш/класс, цена)
EKSK = [
  ("mini-1-3","Мини-экскаватор 1,5–3 т","ковш 0,08–0,12 м³, для узких мест",19600),
  ("mini-5","Мини-экскаватор 5 т","ковш 0,2 м³, гидромолот",21400),
  ("mini-7","Мини-экскаватор 7 т","ковш 0,3 м³, гидромолот",24400),
  ("kolesnyj-160","Экскаватор колёсный 160 серии","ковш 1 м³, город",26600),
  ("kolesnyj-200","Экскаватор колёсный 200 серии","ковш 1 м³, гидромолот",30400),
  ("gusenichnyj-130-190","Экскаватор гусеничный 130–190","ковш 0,6–1 м³",25600),
  ("gusenichnyj-200-220","Экскаватор гусеничный 200–220","ковш 1–1,3 м³",30000),
  ("gusenichnyj-320-330","Экскаватор гусеничный 320–330","ковш 1,5–2 м³, тяжёлый",40200),
  ("pogruzchik-3cx","Экскаватор-погрузчик JCB 3CX","ковш + гидромолот, универсал",26200),
  ("pogruzchik-4cx","Экскаватор-погрузчик JCB 4CX","полный привод, больше ковш",27000),
]

# Самосвалы: (slug, название, объём/особенность, цена)
SAMOSVAL = [
  ("10-14-m3","Самосвал 10–14 м³","городской, узкие дворы",20000),
  ("15-m3","Самосвал 15 м³","оптимален по цене за куб",21400),
  ("20-m3","Самосвал 20 м³","максимальный объём за рейс",23200),
...
from data_blog_daily_20260903 import BLOG_DAILY_20260903   # дневная статья автономного конвейера
from data_blog_daily_20260904 import BLOG_DAILY_20260904   # дневная статья автономного конвейера
from data_blog_daily_20260905 import BLOG_DAILY_20260905   # дневная статья автономного конвейера
from data_blog_daily_20260906 import BLOG_DAILY_20260906   # дневная статья автономного конвейера
...
from data_blog_daily_20260910 import BLOG_DAILY_20260910   # дневная статья автономного конвейера
from data_blog_daily_20260911 import BLOG_DAILY_20260911   # дневная статья автономного конвейера
from data_blog_daily_20260914 import BLOG_DAILY_20260914   # дневная статья автономного конвейера
from data_blog_daily_20260915 import BLOG_DAILY_20260915   # дневная статья автономного конвейера
from data_blog_daily_20260916 import BLOG_DAILY_20260916   # дневная статья автономного конвейера
from data_blog_daily_20260917 import BLOG_DAILY_20260917   # дневная статья автономного конвейера
from data_blog_daily_20260918 import BLOG_DAILY_20260918   # дневная статья автономного конвейера
from data_blog_daily_20260919 import BLOG_DAILY_20260919   # дневная статья автономного конвейера
...
from data_blog_daily_20260926 import BLOG_DAILY_20260926, BLOG_EXTRA_20260926   # 3 статьи дня: ветер, погрузчики, кран на газоне
...
BLOG_EXTRA = dict(BLOG_EXTRA_20260925, **BLOG_EXTRA_20260926)
...
# H1 страниц-услуг по умолчанию собирается как "{Название} краном в Москве и области" —
# для большинства задач это верно (кран — основная техника), но не для всех: вывоз снега
# делает погрузчик с самосвалами, крана в этой задаче вообще нет. Точечный override вместо
# правки формулы для всех: формула и так неточна для части старых страниц (vyvoz-grunta,
# demontazh-zdaniy, ustanovka-stolbov — там тоже не кран), но их H1 уже проиндексирован и
# трогать без отдельного решения не стоит — override закрывает только новую страницу.
TASK_H1_OVERRIDE = {
  "vyvoz-snega": "Вывоз снега в Москве и области",
  "uborka-snega-mtz": "Уборка снега трактором МТЗ в Москве и области",
}
...
TASKS = [
  ...
  ("ustanovka-bytovki","Установка бытовки","установки бытовки",
   ...
   "манипулятор 7–8 т", 20300,
   [("Чем ставить бытовку — краном или манипулятором?","Стандартная бытовка 6×2,4 м весит 1,5–2,5 тонны — её привезёт и поставит манипулятор за один рейс. Обычно хватает манипулятора грузоподъёмностью <a href=\"/manipulyatory/8-tonn/\">7–8 тонн</a>. Кран нужен, если бытовку ставят через здание или на высоту."),
    ("Сколько стоит перевезти и поставить бытовку?","Смена манипулятора — от 20 300 ₽, обычно за смену перевозят 1–3 бытовки по Москве. Точную цену назовём по адресам."),
  ...
  ("podem-na-kryshu","Подъём оборудования на крышу","подъёма оборудования на крышу",
   ...
   "автокран 25 т / автовышка", 19200,
   [("Чем поднять кондиционер на крышу?","Для внешних блоков и сплит-систем обычно хватает автовышки <a href=\"/avtovyshki/21-metrov/\">21</a>–<a href=\"/avtovyshki/32-metrov/\">32 метра</a> или манипулятора. Для тяжёлых чиллеров и вентустановок на высоких зданиях берём автовышку <a href=\"/avtovyshki/40-metrov/\">40 метров</a> или автокран 25–40 тонн — зависит от массы и вылета через здание."),
  ...
  ("razgruzka-fur","Разгрузка фур","разгрузки фур",
   ...
   [("Как быстро подадите технику под разгрузку?","По Москве обычно в течение 2 часов, приём заявок круглосуточно — фуру не придётся держать в простое."),
    ("Чем лучше разгружать?","До 7 тонн на подъём — манипулятор, он же перевезёт груз по площадке. Для более тяжёлых паллет и грузов берём модель <a href=\"/manipulyatory/8-tonn/\">грузоподъёмностью 8 тонн</a>. Тяжелее или с большим вылетом — автокран."),
  ...
  ("ustanovka-septika","Установка септика и ёмкостей","установки септика",
   ...
   [("Чем опустить септик в котлован?","Ёмкости до 5 м³ ставит манипулятор, крупные резервуары — автокран 25 т и выше. Важно, чтобы техника могла подъехать к котловану."),
    ...
    ("Работаете на участках с плохим подъездом?","Есть вездеходные манипуляторы и краны повышенной проходимости для загородных участков. Если вместе с септиком нужно вывезти грунт из котлована — подадим и <a href=\"/samosvaly/vezdehod/\">самосвал-вездеход</a>.")]),
  ...
  ("demontazh-zdaniy","Демонтаж зданий","демонтажа зданий",
   ...
   "экскаватор + самосвалы", 19600,
   [("Какая техника нужна для сноса?","Экскаватор с гидромолотом или крашером для разрушения, погрузчик для сбора и самосвалы для вывоза. Комплект подбираем по объёму объекта: для небольших зданий — колёсный экскаватор <a href=\"/ekskavatory/kolesnyj-160/\">160 серии</a>, для крупных — <a href=\"/ekskavatory/kolesnyj-200/\">200 серии</a> с гидромолотом."),
  ...
  ("vyvoz-grunta","Вывоз грунта","вывоза грунта",
   ...
   "экскаватор + самосвалы 20 м³", 23200,
   [("Как считается вывоз грунта?","По объёму в кубометрах и расстоянию до полигона. Самосвал 20 м³ вывозит примерно 12–14 тонн грунта за рейс."),
    ...
    ("Можно только самосвалы, без экскаватора?","Да, если у вас есть своя погрузочная техника — подадим только самосвалы. Если подъезд грунтовый или после дождя раскис, подойдёт <a href=\"/samosvaly/vezdehod/\">самосвал-вездеход</a> повышенной проходимости.")]),
  ("vyvoz-snega","Вывоз снега","вывоза снега",
   "Расчистка и вывоз снега с придомовых территорий, парковок и промплощадок. Погрузчик и самосвалы вывозят снег на согласованный пункт приёма — разово или на весь сезон.",
   "погрузчик + самосвалы 20 м³", 23200,
   [("Убираете снег разово или только на сезон?","И так, и так. Разовый вызов — по факту снегопада, но в разгар зимы возможна очередь. Сезонный договор с фиксированным окном подачи выгоднее и надёжнее для УК и коммерческих объектов."),
    ("Куда вывозите снег?","На согласованный снегоприёмный пункт — это не то же самое, что полигон для грунта или мусора. Если своего пункта нет, поможем с ближайшим по направлению."),
    ("Как считается объём снега?","По площади и толщине покрова с коэффициентом разрыхления: свежий снег — около ×1,25, слежавшийся и с реагентами — выше. Дальше объём делится на рейсы самосвала."),
    ("Работаете в Балашихе и окрестностях?","Да, восточное направление — Балашиха, Новокосино, Заря, Старая Купавна, Медвежьи озёра — обслуживаем регулярно, в том числе на сезонных договорах на всю зиму.")]),
  ("uborka-snega-mtz","Уборка снега трактором МТЗ","уборки снега трактором МТЗ",
   "Трактор МТЗ с дорожной щёткой сметает свежий снег с дороги или парковки за один-два прохода — быстрее, чем ждать погрузчик с самосвалами. Первый этап перед вывозом снега, не замена ему.",
   "МТЗ-82 с дорожной щёткой", 18400,
   [("Чем щётка лучше вывоза снега самосвалами?","Не лучше — это разные задачи. Щётка быстро освобождает дорогу или площадку, сдвигая снег к обочине. Если валы нужно убрать полностью, дальше подключаем <a href=\"/uslugi/vyvoz-snega/\">вывоз снега</a> погрузчиком и самосвалами."),
    ("На любом снегу работает?","Лучше всего — на свежем и тонком. Уплотнённый за несколько дней снег с реагентами щётка не сметает чисто, для него нужен повторный проход или другая техника."),
...
# nom (именительный) -> slug, для перелинковки соседних гео-страниц друг с другом
# прямо в тексте "куда ещё выезжаем" (см. linkify_geo_neighbors) — соседи
# перечислены как обычный текст городов, часть из них сами являются страницами
# сайта, и раньше не были ссылками.
GEO_NOM_TO_SLUG = {g[2]: g[0] for g in GEO}

def linkify_geo_neighbors(neigh, active_slug):
    """В списке соседних населённых пунктов (через запятую) превращает те имена,
    которые совпадают с именительным падежом другой гео-страницы сайта, в ссылку
    на неё. Остальные имена (посёлки/города без своей страницы) остаются текстом."""
    parts = [p.strip() for p in neigh.split(",")]
    out = []
    for p in parts:
        slug = GEO_NOM_TO_SLUG.get(p)
        if slug and slug != active_slug:
            out.append('<a href="/geo/%s/">%s</a>' % (slug, esc(p)))
        else:
            out.append(esc(p))
    return ", ".join(out)

# Условия, на которых указаны цены. Ничего нового здесь не утверждается:
#   · состав смены — из ответа «Что входит в стоимость смены?», который уже
#     стоит в FAQ на страницах техники;
#   · дата актуальности — из pricing.md, строка «Последнее обновление»;
#   · оговорка про оферту — прайс «от» с ценой, зависящей от адреса и срока,
#     публичной офертой по ст. 437 ГК РФ не является, и сказать это нужно
#     прямо, иначе цифра в таблице читается как твёрдое обязательство.
# Если владелец меняет прайс — здесь меняется дата, иначе оговорка врёт.
PRICE_UPDATED = "2026-09-14"
PRICE_UPDATED_HUMAN = "14 сентября 2026"
...
def price_terms():
    return (
      ...
      '<div class="price-terms">'
        ...
        '</div>'
        '<p class="price-terms__note">Цены указаны «от», за смену. Итоговая стоимость '
        'зависит от адреса подачи, срока аренды и задачи, поэтому прайс на сайте '
        'не является публичной офертой — точную цену называем по телефону или в ответ '
        'на заявку. Цены актуальны на <time datetime="%s">%s</time>.</p>'
      '</div>' % (PRICE_UPDATED, PRICE_UPDATED_HUMAN))
...
def tonnage_prose(tn, pr):
    ...
    # Раньше здесь стояла ПОЛНАЯ таблица цен по всем 10 классам техники — она
    # уже есть на /avtokrany/ и дублировалась слово в слово на каждой из 10
    # тоннажных страниц (аудит: ~54% текста страницы — идентичный блок).
    # Цена этого класса названа абзацем выше; здесь — только ссылка на полное
    # сравнение, чтобы не разбавлять уникальный контент страницы шаблоном.
    h += '<p><a href="/avtokrany/">Сравнить цены по всем грузоподъёмностям →</a></p>'
...
def geo_mo_prose(slug, prep, neigh):
    ...
    out += "<p>Кроме работ в %s обслуживаем ближние точки: %s. "\
           ...
           esc(prep), linkify_geo_neighbors(neigh, slug))
...
def task_prose(slug, tech, faqs):
    ...
    d = TASKS_DATA[slug]
    out = ""
    # intro иногда намеренно содержит доверенную ссылку <a href="..."> на другую
...
DESC_LIMIT  = 155     # верхняя граница, после которой Google обрезает
...
# Хабовый FAQ — раньше один и тот же блок из 4 вопросов дословно повторялся
# на /avtokrany/, /avtovyshki/, /manipulyatory/, /ekskavatory/, /samosvaly/
# (внешний SEO-аудит, 31.08.2026: 100% совпадение текста и разметки FAQPage).
# Вопросы и ответы здесь — про то, что у каждого вида техники устроено
# по-своему; общие организационные вопросы (ЭДО, минимальный заказ) остались
# только там, где для них есть особенность (иначе смысла разводить нет).
HUB_FAQS = {
  "avtokrany": [
    ("Как понять, какая грузоподъёмность нужна?", "По массе груза и расстоянию от места, где встанет кран, до точки подъёма — это расстояние (вылет) снижает грузоподъёмность сильнее, чем кажется. Назовите груз и примерную схему объекта, подберём тоннаж и сразу покажем цену на странице этого класса."),
    ("Нужен ли пропуск для въезда в Москву?", "Для машин тяжелее 12 тонн — да, в пределах МКАД действует ограничение для грузового транспорта. Пропуск оформляем сами, заявку по крупному крану лучше подавать за 2–3 дня, по мелкому — в день обращения."),
    ("Такелажники и стропы — отдельно?", "Стропы и обвязка входят в подачу крана. Отдельная бригада такелажников или стропальщик на объекте — по заявке, если у вас его нет; предупредите при заказе, подберём кран с учётом этого времени в смене."),
    ("Как быстро подадите кран на объект?", "По Москве и области — в день обращения, в среднем за 2–3 часа для тоннажа до 40 тонн. Тяжёлую и гусеничную технику, а также машины с пропуском, лучше заказывать на следующий день."),
  ],
  "avtovyshki": [
    ("Телескопическая или коленчатая — какая нужна?", "Телескопическая берёт выше по прямой и годится для фасадов без препятствий. Коленчатая заводит люльку за угол здания, под козырёк, через провода и деревья — там, где по прямой не подобраться. Скажите, что за объект, подскажем."),
    ("Сколько человек и груза выдержит люлька?", "Обычно 2 человека и до 200–250 кг инструмента и материала — точная цифра у каждой модели своя, указана на странице высоты. Если планируете поднимать что-то тяжёлое (оконный блок, кондиционер), скажите вес заранее."),
    ("Нужен ли допуск на работу на высоте?", "У ваших рабочих, которые поднимаются в люльке, — да, это ваша ответственность как заказчика (удостоверение и допуск к высотным работам). Оператор вышки — наш сотрудник и отдельного допуска от вас не требует."),
    ("Работаете зимой и в дождь?", "В мороз без ограничений. В сильный дождь, гололёд и при ветре свыше 12–15 м/с работы на высоте останавливаем по технике безопасности — это решает оператор на месте, не диспетчер по телефону."),
  ],
  "manipulyatory": [
    ("Чем манипулятор выгоднее крана и грузовика по отдельности?", "Один выезд вместо двух: манипулятор сам довозит груз и сам его снимает краном-стрелой, не нужно отдельно заказывать грузовик и отдельно кран под разгрузку. Экономия — на подаче второй машины и на времени простоя груза на объекте."),
    ("Какой максимальный вес на борт и на стрелу?", "Борт (кузов) обычно берёт больше, чем стрела: например, до 20 тонн на платформу и 7–8 тонн на крюк стрелы за один подъём. Если груз тяжелее — подскажем, что можно разбить на два подъёма, а что уже требует автокрана."),
    ("Заедет ли манипулятор на участок без асфальта?", "Стандартная машина — по укатанному грунту в сухую погоду. Для рыхлого грунта, снега или узкого проезда есть вездеходные версии на полноприводном шасси — уточните состояние подъезда при заказе."),
    ("Разгрузка и расстановка груза входят в цену?", "Да, подъём и расстановка груза на месте — часть подачи манипулятора, отдельно это не считается. Отдельно — только увеличенное расстояние от машины до точки установки, если стрела не достаёт."),
  ],
  "ekskavatory": [
    ("Мини, колёсный или гусеничный — как выбрать?", "Мини — для узких дворов и траншей под коммуникации, заезжает в проём калитки. Колёсный — для города: сам едет по дорогам, не нужен трал. Гусеничный — для больших объёмов и тяжёлого грунта, но доставляется тралом отдельно."),
    ("Гидромолот — отдельная заявка?", "Да, гидромолот на экскаватор заказывается отдельно и не на все модели класса подходит по весу машины — скажите задачу (снос бетона, скальный грунт), подберём модель сразу с ним."),
    ("Вывозите грунт или только копаете?", "Сам экскаватор грунт не вывозит — это отдельная услуга самосвалом, можно заказать вместе одной заявкой. Если нужен только вывоз без земляных работ, смотрите страницу самосвалов."),
    ("Как гусеничный экскаватор попадёт на объект?", "Своим ходом гусеничная техника по городу не ездит — везём тралом, это отдельная строка в смете (перевозка туда и обратно). Колёсные модели и мини-экскаваторы едут своим ходом или на прицепе, дешевле."),
  ],
  "samosvaly": [
    ("Сколько рейсов делает самосвал за смену?", "Зависит от расстояния до полигона: по Москве и ближней области обычно 6–10 рейсов за смену на кузове 10–20 м³. Точное число посчитаем, когда назовёте адрес объекта и куда вывозить."),
    ("Куда вывозите грунт и мусор?", "На лицензированные полигоны и карьеры-приёмники — по строительному мусору и грунту это обязательное требование, работаем только так. По запросу даём талон/накладную, подтверждающую вывоз именно туда."),
    ("Считаете по рейсам или по смене?", "По умолчанию — сменой (8–10 часов), это выгоднее при вывозе от 5 рейсов. Разовый вывоз 1–2 рейсов можно посчитать поштучно — скажите объём, предложим, что дешевле в вашем случае."),
    ("Нужны ли документы для вывоза грунта?", "Талон на вывоз с указанием объёма и точки приёма — стандартная часть услуги, оформляем сами. Если объекту нужен отдельный договор или акт для отчётности перед заказчиком/УК — тоже сделаем, скажите при заявке."),
  ],
}
GENERIC_HUB_FAQ = [
...
def build():
    ...
    for t in TYPES:
        ...
        faqs = HUB_FAQS.get(t["slug"], GENERIC_HUB_FAQ)
        if t["slug"]=="avtokrany":
            # Лид уже показан в hero() как .lead — здесь не повторяем его вторым <p>.
            prose = ('<h2>Аренда автокрана по грузоподъёмности</h2>'
                     ...
                     '<h2>Частые вопросы</h2>%s') % (tonnage_table(),
            ...
            # Лид уже показан в hero() как .lead — здесь не повторяем его вторым <p>.
            prose = ('<h2>Цены на аренду</h2><p>Стоимость указана «от», за смену — включает работу оператора и топливо. Точную цену под вашу задачу назовём по телефону.</p>%s'
                     '<h2>Частые вопросы</h2>%s') % (
                     price_table_types(t["slug"]), faq_html(faqs))
    ...
    for slug, name, txt in MARKI:
        ...
        ld = [breadcrumb_ld(crumbs), service_ld(h1, lead, 24000), faq_ld(d["faqs"]), local_business_ld()]
    ...
    ld = [breadcrumb_ld(crumbs), service_ld("Услуги спецтехники","Аренда спецтехники под конкретные задачи в Москве и области",24000), faq_ld(faqs_u), local_business_ld()]
    page("uslugi/index.html", seo_title("Услуги спецтехники", "в Москве",
         ...
         crumbs, hero("Услуги", "Услуги спецтехники под вашу задачу",
                      # Числа — из таблицы услуг ниже на этой же странице:
                      # монтаж ангара идёт автокраном 25–50 т, установка бытовки —
                      # манипулятором 7–8 т.
                      "Не нужно разбираться, какой кран заказать — опишите задачу, подберём технику и назовём точную цену. Автокраны 25–50 тонн и манипуляторы 7–8 тонн, работаем по Москве и всей области круглосуточно.", 24000),
    ...
    for slug, prep, nom, route, neigh, spec in GEO:
        ...
        ld = [breadcrumb_ld(crumbs), service_ld("Аренда спецтехники в %s" % prep, lead, 24000, area=nom), faq_ld(d["faqs"]), local_business_ld()]
        page("geo/%s/index.html" % slug, title, desc, crumbs,
             hero("Зона работы", h1, d["angle"], 24000), body(prose, rel) + TRUST, ld)
        pages += 1
